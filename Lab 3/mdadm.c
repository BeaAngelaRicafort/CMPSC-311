...
int mdadm_read(uint32_t addr, uint32_t len, uint8_t *buf) {
	...
	if ((addr + len) > 1048576) { // Out-of-Bound Linear Address (Fail)
		return -1;
	}
...
int mdadm_write(uint32_t addr, uint32_t len, const uint8_t *buf) {
	...
	while (total_bytes_written < len) {
		...
		uint32_t seek_to_disk = make_op(JBOD_SEEK_TO_DISK, 0, disk_id); // Make Locate Disk Command
		if (jbod_operation(seek_to_disk, NULL) != 0) { // Failed to Seek to Disk
			return -1;
		}

		uint32_t seek_to_block = make_op(JBOD_SEEK_TO_BLOCK, block_id, 0); // Make Locate Block Command
		if (jbod_operation(seek_to_block, NULL) != 0) { // Failed to Seek to Block
			return -1;
                }
		...
		int bytes_written = JBOD_BLOCK_SIZE - offset; // Number of Bytes Written in Current Interation of While Loop
		
		if (bytes_written > len - total_bytes_written) { // No Buffer Overflow
			bytes_written = len - total_bytes_written;
		}

		memcpy(temp_buff + offset, buf + total_bytes_written, bytes_written);
		...
		uint32_t write_block = make_op(JBOD_WRITE_BLOCK, 0, 0); // Make Write Command
		if (jbod_operation(write_block, temp_buff) != 0) { // Failed to Write
			return -1;
		}
...
/*CWD /home/cmpsc311/cmpsc311/assignments/sp25-lab3-BeaAngelaRicafort */
/*CWD /home/cmpsc311/cmpsc311/assignments/sp25-lab3-BeaAngelaRicafort */
/*CWD /home/cmpsc311/cmpsc311/assignments/sp25-lab3-BeaAngelaRicafort */
/*CWD /home/cmpsc311/cmpsc311/assignments/sp25-lab3-BeaAngelaRicafort */
/*CWD /home/cmpsc311/cmpsc311/assignments/sp25-lab3-BeaAngelaRicafort */
