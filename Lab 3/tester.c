...
int main(int argc, char *argv[])
{
  ...
  int score = 0;
 printf("/********************************************************************/\n");
  printf("Please ensure there were no warnings when you compiled your program\n");
  printf("If you have any make errors or warnings then you will receive a straight 0.\n");
  printf("Please ensure there are sufficent comments in your code\n");
  printf("Do not foget to add your name and date in comments on top of your mdadm.c\n");
  printf("/********************************************************************/\n");
  ptr=fopen("mdadm.c","a+");
  ...
  if (getcwd(cwd, sizeof(cwd)) != NULL) {
   ...
   fclose(ptr);
  test_mount_unmount();
  test_read_before_mount();
  test_read_invalid_parameters();
  test_read_within_block();
